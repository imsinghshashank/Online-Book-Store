...
<body>
            ...
            <c:choose>
                ...
                <c:otherwise>
                    <c:if test="${!sessionScope.isAdminLoggedIn}">
                        <%-- User login/signup links are not needed on admin login page, and should be hidden if admin is logged in --%>
                    </c:if>
                </c:otherwise>
            </c:choose>
            <c:choose>
                ...
                <c:otherwise>
                    <c:if test="${!sessionScope.isUserLoggedIn}">
                        <%-- Admin login link is not needed on admin login page, but we ensure it's hidden if user is logged in --%>
                    </c:if>
                </c:otherwise>
            </c:choose>
        </nav>
    ...
    <div class="login-container">
        ...
        <c:if test="${!sessionScope.isUserLoggedIn}">
            <form action="/admin/login" method="post" class="w-full max-w-sm">
        ...
        <c:if test="${sessionScope.isUserLoggedIn}">
            <p class="text-center text-lg text-gray-700">You are logged in as a user. Please log out of your user session to access admin functionalities.</p>
            <p class="mt-4 text-center">
                <a href="/logout" class="text-blue-600 hover:underline">Logout User</a>
            </p>
        </c:if>
    </div>
